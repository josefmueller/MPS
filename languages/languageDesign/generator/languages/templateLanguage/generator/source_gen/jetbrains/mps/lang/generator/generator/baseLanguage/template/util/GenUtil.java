...
public class GenUtil {
  ...
  public static int compareQueryNodes(SNode a, SNode b) {
    if (a == b) {
      return 0;
    }

    SNode aRoot = SNodeOperations.as(SNodeOperations.getContainingRoot(a), MetaAdapterFactory.getInterfaceConcept(0xceab519525ea4f22L, 0x9b92103b95ca8c0cL, 0x110396eaaa4L, "jetbrains.mps.lang.core.structure.INamedConcept"));
    SNode bRoot = SNodeOperations.as(SNodeOperations.getContainingRoot(b), MetaAdapterFactory.getInterfaceConcept(0xceab519525ea4f22L, 0x9b92103b95ca8c0cL, 0x110396eaaa4L, "jetbrains.mps.lang.core.structure.INamedConcept"));
    assert (aRoot != null) && (bRoot != null) : "a:" + SNodeOperations.getContainingRoot(a) + "; b:" + SNodeOperations.getContainingRoot(b);

    int rootCmp = SPropertyOperations.getString(aRoot, MetaAdapterFactory.getProperty(0xceab519525ea4f22L, 0x9b92103b95ca8c0cL, 0x110396eaaa4L, 0x110396ec041L, "name")).compareTo(SPropertyOperations.getString(bRoot, MetaAdapterFactory.getProperty(0xceab519525ea4f22L, 0x9b92103b95ca8c0cL, 0x110396eaaa4L, 0x110396ec041L, "name")));
    if (rootCmp != 0) {
      return rootCmp;
    }

    // compare nodes inside same root by "path" 
    List<SNode> aa = ListSequence.fromList(SNodeOperations.getNodeAncestors(a, null, true)).reversedList();
    List<SNode> ba = ListSequence.fromList(SNodeOperations.getNodeAncestors(b, null, true)).reversedList();

    int index = 0;
    while (index < ListSequence.fromList(aa).count() && index < ListSequence.fromList(ba).count()) {
      if (ListSequence.fromList(aa).getElement(index) != ListSequence.fromList(ba).getElement(index)) {
        return SNodeOperations.getIndexInParent(ListSequence.fromList(aa).getElement(index)) - SNodeOperations.getIndexInParent(ListSequence.fromList(ba).getElement(index));
      }
      index++;
    }

    return (index == ListSequence.fromList(aa).count() ? 1 : -1);
  }
